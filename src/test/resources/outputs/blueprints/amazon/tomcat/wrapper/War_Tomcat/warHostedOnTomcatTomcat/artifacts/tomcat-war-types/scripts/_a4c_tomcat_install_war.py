...
env_map['TOMCAT_PORT'] = get_property(ctx.target, 'tomcat_port')
env_map['TOMCAT_HOME'] = get_property(ctx.target, 'tomcat_home')
env_map['CONTEXT_PATH'] = get_property(ctx.source, 'context_path')
...
parsed_output = execute(ctx.download_resource('artifacts/tomcat-war-types/scripts/tomcat_install_war.sh'), new_script_process, operationOutputNames)
...
ctx.source.instance.runtime_properties['local_application_url'] = r'http://' + get_attribute(ctx.source, 'ip_address') + r':' + get_property(ctx.source, 'tomcat_port') + r'/' + get_property(ctx.source, 'context_path')
ctx.source.instance.runtime_properties['application_url'] = r'http://' + get_attribute(ctx.source, 'public_ip_address') + r':' + get_property(ctx.source, 'tomcat_port') + r'/' + get_property(ctx.source, 'context_path')
ctx.source.instance.update()
ctx.target.instance.runtime_properties['server_url'] = r'http://' + get_attribute(ctx.target, 'public_ip_address') + r':' + get_property(ctx.target, 'tomcat_port')
