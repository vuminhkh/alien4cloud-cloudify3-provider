...
@with_neutron_client
@with_cinder_client
def create(nova_client, neutron_client, cinder_client, args, **kwargs):
...
@with_nova_client
@with_cinder_client
def start(nova_client, cinder_client, start_retry_interval, private_key_path, **kwargs):
...
@with_nova_client
@with_cinder_client
def stop(nova_client, cinder_client, **kwargs):
    if is_external_resource(ctx):
...
@with_neutron_client
@with_cinder_client
def delete(nova_client, neutron_client, cinder_client, **kwargs):
...
def create_floatingip(neutron_client, resource_name, property):
    ...
    if 'floatingip' in property:
        floatingip.update(property['floatingip'])
    else:
        floatingip.update({})
